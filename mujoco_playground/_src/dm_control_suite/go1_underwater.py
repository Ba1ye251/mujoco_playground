...
# ==============================================================================
"""Go1 underwater swimming environment."""
...
class Swim(mjx_env.MjxEnv):
  ...
  def _post_init(self) -> None:
    ...
    self._head_geom_id = self._mj_model.site("head").id
    self._target_mocap_body_id = self._mj_model.body("target").mocapid[0]
    self._radii = self._mj_model.geom_size[self._target_geom_id, 0] + 0.1
    self._min_xyz = jp.array([-2.0, -2.0, 0.1])
    self._max_xyz = jp.array([2.0, 2.0, 1.0])
  ...
  def _get_obs(self, data: mjx.Data, info: dict[str, Any]) -> jax.Array:
    ...
    head_to_target_global = (
        data.geom_xpos[self._target_geom_id]
        - data.site_xpos[self._head_geom_id]
    )
    head_to_target_local = (
        head_to_target_global @ data.xmat[self._torso_body_id]
    )
  ...
  def _get_reward(
    ...
    in_target = reward.tolerance(
        jp.linalg.norm(head_to_target_local),
